...
const List = ({ isOpenModal }) => {
 ...
 function getItems() {
  axios.get(link)
    .then(lists => setItems(lists.data.items))
   ...
   <div className=" flex flex-wrap py-16" >
    {
     items.slice(0,sliceUpperLimit).map((item, index) =>
      <div className="w-80 py-6" key={item.sys.id.toString()}>{item.sys.id}
        <Card isOpenModal={isOpenModal} price={item.price} title={item.title} id={item.id} image={item.image} />
       {((item.sys.id + 1) % 3 == 0) && <div ref={setElement}/>}
      </div>
     )
    }
    </div>
